...
function register(req,res){
  ...
  var mongoUser = {username:aUsername , withFacebook:"no"}
...
const putPassword = (req,res) => {
  ...
  console.log("put password username "+username)
...
//"jmb27"
  ...
  if (err){
    ...
    if(doc){
      ...
      Users.findOneAndUpdate({username:username},{hash:newHash},{new:true},(err,doc)=>{
        if (err){
        //res.status(400).send({error:err})
        console.log("nope")
        }
...
passport.use(new FacebookStrategy({
    ...
    if (profile){
      ...
      Profiles.findOne({ email: profile.email}, function(err, doc){
      if (err){
      console.log("nope, an error")
        ...
        if (!err &&!doc){
         ...
         Users.create({ theId:" ", username: profile.displayName, salt: " ", hash:" ", withFacebook:"yes" }, function (err, small) {
           if (err) return handleError(err);
            // saved!
           });
        }

        var mongoProf = {username: profile.dislayName, headline:"Today is the best day ever.", avatar:"" ,following:[], theId:"", email:profile.email, dob:11021998, zipcode:""}

         var newProfile2 = new Profiles( mongoProf)

     //myPosts[posts].append(newArticle)
      
      newProfile2.save((function (err, result) {
        if (err) {
          console.log("whoopsies")
          res.send("error")
        }
          ...
          console.log("yayyayay, new Profile uploaded")
          //facebookEmail = doc.id
          console.log("result "+result)
          this.facebookProfile = result.id
          console.log(this.facebookProfile)
          //res.json(doc.id);
...
module.exports = (app) => {
  ...
  */
  ...
  app.get('/auth/facebook/callback',
    ...
    function(req, res) {
        res.redirect('/#/main/'+this.facebookProfile);
    });  
